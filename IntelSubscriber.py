...
class IntelSubscriber(Node):
    def __init__(self):     
      ...
      #       10)
       ...
       self.ts.registerCallback(self.IntelSubscriberRGB)
   ...
   #         self.get_logger().error(f"Failed to convert depth image: {e}")
    ...
    def IntelSubscriberRGB(self,rgb_msg:Image,info_msg:CameraInfo,depth_msg:Image):
       ...
       t_k = time.time()
       print(t_k)
       self.get_logger().info(f"Received caminfo: {info_msg.data}")
       ...
       corners, ids, _ = aruco_detector.detectMarkers(rgb_image, aruco_dict, parameters=aruco_params)
       if ids is not None:
